<template>
  <div class="app-container">
    ...
    <ModalDialog
      :title="dialogs.preOutBound.title"
      :name="dialogs.preOutBound.name"
      :visible.sync="dialogs.preOutBound.visible"
      @afterClosed="onPreOutboundClose"
      :optional="optional"
    >
      <h3>{{ preOutBound.message }}</h3>
    </ModalDialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    onPerInvoke() {
      preOutBound(false).then((resp) => {
        if (resp.status == "OK") {
          this.preOutBound = resp.message;
          this.dialogs.preOutBound.visible = true;
        }
      });
    },
    onSettingClose(val) {
