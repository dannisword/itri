<template>
  <div class="app-container">
    ...
    <ModalDialog
      :title="dialogs.pre.title"
      :name="dialogs.pre.name"
      :visible.sync="dialogs.pre.visible"
      @afterClosed="onPreInventoryClose"
      :optional="optional"
    >
      <h3>{{ preInventory.message }}</h3>
    </ModalDialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    onPerInvoke() {
      preInventory(false).then((resp) => {
        if (resp.status == "OK") {
          this.preInventory = resp.message;
          this.dialogs.pre.visible = true;
        }
      });
    },
    onSettingClose(val) {
